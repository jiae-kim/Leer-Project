...
public class CommunityDao {
	...
	public CommunityDao() {
		try {
			prop.loadFromXML(
					new FileInputStream(CommunityDao.class.getResource("/db/sql/comuBoard-mapper.xml").getPath()));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public ArrayList<Category> selectCategoryList(Connection conn) {
		...
		try {
			...
			while (rset.next()) {
				cateList.add(new Category(rset.getInt("category_no"), rset.getString("category_name")));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public ComuBoard selectBoard(Connection conn, int comuNo) {
		...
		try {
			...
			if (rset.next()) {
				...
				c = new ComuBoard(rset.getInt("comu_no"), rset.getString("nickname"), rset.getString("category_no"),
						rset.getString("tag"), rset.getString("title"), rset.getDate("enroll_date"),
						rset.getInt("like_count"), rset.getInt("comment_count"), rset.getInt("view_count"),
	...
	public ArrayList<Reply> selectReplyList(Connection conn, int comuNo) {
		...
		try {
			...
			while (rset.next()) {
				list.add(new Reply(rset.getInt("comm_no"), rset.getString("nickname"), rset.getString("comm_content"),
	...
	public ArrayList<ComuBoard> selectMyBoardList(Connection conn, PageInfo pi, int memNo) {
		...
		try {
			...
			while (rset.next()) {
				...
				list.add(new ComuBoard(rset.getInt("comu_no"), rset.getString("nickname"), rset.getString("title"),
						rset.getDate("enroll"), rset.getInt("view_count")));
	...
	public ArrayList<Notice> selectNotiBoardList(Connection conn) {
		...
		try {
			...
			while (rset.next()) {
				list.add(new Notice(rset.getInt("noti_no"), rset.getString("nickname"), rset.getString("title"),
						rset.getDate("enroll"), rset.getInt("noti_views")));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public Notice selectNotice(Connection conn, int notiNo) {
		...
		try {
			...
			if (rset.next()) {
				...
				n = new Notice(rset.getInt("noti_no"), rset.getString("nickname"), rset.getString("title"),
						rset.getString("content"), rset.getDate("enroll"), rset.getInt("noti_views"));
	...
	public int selectCategoryListCount(Connection conn, int cNo) {
		int listCount = 0;
	...
	public ArrayList<ComuBoard> selectTagList(Connection conn, PageInfo pi, String tag) {
		ArrayList<ComuBoard> list = new ArrayList<>();
	...
	public int selectTagListCount(Connection conn, String tag) {
		int listCount = 0;
		...
		String sql = prop.getProperty("selectTagListCount");
